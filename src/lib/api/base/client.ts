...
class ApiClient {
  ...
  private setupInterceptors() {
    ...
    this.instance.interceptors.request.use((config) => {
      ...
      try {
        ...
        alert("Error accessing auth store:" + error);
      }
    ...
    this.instance.interceptors.response.use(
      ...
      (error: AxiosError) => {
        ...
        return Promise.reject(
          new Error("Error de conexión. Verifica tu red e intenta nuevamente.")
        );
      }
    );
  }
  ...
  private getCustomErrorMessage(
    ...
    if (status === 400 && url?.includes("/auth/register")) {
      if (
        originalMessage?.toLowerCase().includes("email") ||
    ...
    if (
      ...
      originalMessage?.toLowerCase().includes("license plate")
    ) {

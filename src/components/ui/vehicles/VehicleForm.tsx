...
export const VehicleForm: React.FC<VehicleFormProps> = ({ mode, vehicle, onSuccess, onCancel }) => {
  ...
  const {
    ...
    resolver: zodResolver(mode === 'edit' ? editVehicleSchema : createVehicleSchema),
    defaultValues:
      mode === 'edit' && vehicle
        ? {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      let success = false
      if (mode === 'create') {
        const result = await createVehicle(vehicleData as CreateVehicleFormData)
        success = result !== null
      } else if (mode === 'edit' && vehicle) {
        success = await updateVehicle(vehicle.id, vehicleData as EditVehicleFormData)
      }
    ...
    <div className='glass rounded-4xl p-8'>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
          <div>
            <label className='block text-sm font-medium text-secondary-700 mb-2'>Modelo del Vehículo *</label>
            <input
          ...
          <div>
            <label className='block text-sm font-medium text-secondary-700 mb-2'>Placa *</label>
            <input
          ...
          <div>
            <label className='block text-sm font-medium text-secondary-700 mb-2'>Precio por Día (COP) *</label>
            <input
              ...
              placeholder='150000'
            />
            {errors.daily_price && <p className='text-red-500 text-sm mt-1'>{errors.daily_price.message}</p>}
          </div>
        </div>
        ...
        {mode === 'edit' && (
          <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            <div>
              <label className='block text-sm font-medium text-secondary-700 mb-2'>Clase</label>
              <input
                {...register('class')}
                type='text'
                className='glass w-full rounded-xl p-3 border border-secondary-200 focus:border-primary-400 focus:outline-none transition-colors'
              ...
              {mode === 'edit' && editErrors.class && (
                <p className='text-red-500 text-sm mt-1'>{editErrors.class.message}</p>
              )}
            </div>

            <div>
              <label className='block text-sm font-medium text-secondary-700 mb-2'>Tracción</label>
              <input
                {...register('drive')}
                type='text'
                className='glass w-full rounded-xl p-3 border border-secondary-200 focus:border-primary-400 focus:outline-none transition-colors'
              ...
              {mode === 'edit' && editErrors.drive && (
                <p className='text-red-500 text-sm mt-1'>{editErrors.drive.message}</p>
              )}
            </div>

            <div>
              <label className='block text-sm font-medium text-secondary-700 mb-2'>Tipo de Combustible</label>
              <input
                {...register('fuel_type')}
                type='text'
                className='glass w-full rounded-xl p-3 border border-secondary-200 focus:border-primary-400 focus:outline-none transition-colors'
              ...
              {mode === 'edit' && editErrors.fuel_type && (
                <p className='text-red-500 text-sm mt-1'>{editErrors.fuel_type.message}</p>
              )}
            </div>

            <div>
              <label className='block text-sm font-medium text-secondary-700 mb-2'>Transmisión</label>
              <input
              ...
              {mode === 'edit' && editErrors.transmission && (
                <p className='text-red-500 text-sm mt-1'>{editErrors.transmission.message}</p>
              )}
            </div>
          </div>
        )}
        ...
        <div>
          <label className='block text-sm font-medium text-secondary-700 mb-2'>Condiciones de Renta *</label>
          <textarea
          ...
          />
          {errors.rental_conditions && <p className='text-red-500 text-sm mt-1'>{errors.rental_conditions.message}</p>}
        </div>
        ...
        <div>
          ...
          <div className='space-y-4'>
            ...
            >
              {({ open }) => (
                ...
                >
                  ...
                  {isUploading ? 'Subiendo...' : 'Seleccionar Fotos del Vehículo'}
                </Button>
              )}
            </CldUploadWidget>
            ...
            <div className='flex items-center justify-between text-sm text-secondary-600'>
              ...
              {uploadedImages.length > 0 && <span className='text-green-600'>✓ Listo para continuar</span>}
            </div>
            ...
            {errors.url_photos && <p className='text-red-500 text-sm'>{errors.url_photos.message}</p>}
          </div>
        </div>

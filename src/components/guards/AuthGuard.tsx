...
export const AuthGuard: React.FC<AuthGuardProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (routeType === 'protected') {
      if (!hasValidSession) {
        ...
        if (!token) {
          // No hay token - primera vez o sesión completamente expirada
          console.warn('AuthGuard: No hay token, redirigiendo a login')
          router.replace(
            `${AUTH_ROUTES_CONFIG.DEFAULTS.LOGIN}?reason=no-token&callbackUrl=${encodeURIComponent(pathname)}`
          )
          return
        } else if (token && !user) {
          // Hay token pero no hay usuario - token inválido o expirado
          console.warn('AuthGuard: Token presente pero usuario inválido, redirigiendo a access-denied')
          router.replace(`${AUTH_ROUTES_CONFIG.DEFAULTS.ACCESS_DENIED}?reason=invalid-token`)
          return
        } else if (!isAuthenticated) {
          // Usuario no autenticado - problemas de permisos
          console.warn('AuthGuard: Usuario sin permisos, redirigiendo a access-denied')
          router.replace(`${AUTH_ROUTES_CONFIG.DEFAULTS.ACCESS_DENIED}?reason=unauthorized`)
          return
        } else {
          // Caso genérico - sesión inválida
          console.warn('AuthGuard: Sesión inválida en ruta protegida, redirigiendo a access-denied')
          router.replace(`${AUTH_ROUTES_CONFIG.DEFAULTS.ACCESS_DENIED}?reason=session-invalid`)
          return
        }
      }
    }
    ...
    if (routeType === 'auth') {
      if (hasValidSession) {
        // Usuario ya autenticado intentando acceder a rutas de auth
        console.info('AuthGuard: Usuario autenticado redirigido desde ruta de auth')
  ...
  if (!_hasHydrated || isVerifying) {
    const routeType = _hasHydrated ? RouteUtils.getRouteType(pathname) : 'unknown'
    ...
    return (
      <div className='min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-50 via-white to-accent-50'>
        <div className='text-center max-w-md mx-auto px-4'>
          ...
          {/* Brand */}
          <div className='mb-6'>
            <h2 className='text-2xl font-semibold gradient-text mb-2'>CarLink</h2>
          ...
          <div className='space-y-4'>
            ...
            <div className='glass rounded-2xl p-4'>
              <div className='flex items-center justify-center space-x-2 text-sm text-secondary-600'>
                ...
                <span>{!_hasHydrated ? 'Restaurando sesión...' : 'Verificando acceso...'}</span>
              </div>
            </div>
          </div>
          ...
          <div className='mt-6 text-xs text-secondary-400'>
            <p>{!_hasHydrated ? 'Verificando credenciales almacenadas' : 'Validando permisos de usuario'}</p>
          </div>
        </div>
      </div>
    )
